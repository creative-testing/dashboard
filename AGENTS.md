...
`master` powers the live Meta Ads dashboard—never push or merge into it without explicit owner approval. Daily work happens on `saas-mvp`; create topic branches from there and open PRs back into `saas-mvp`. Production data comes from GitHub Actions (boss dashboard) and VPS cron jobs (SaaS), so avoid editing `data/optimized` manually; use the refresh scripts instead. When deploying, keep the two GitHub workflows separate: use the fast code-only deploy for UI tweaks and the auto-refresh workflow for data updates.
