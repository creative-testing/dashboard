...
def transform_data(input_dir, output_dir):
    ...
    # Get reference date from baseline first, then hybrid files
    reference_date = None
    
    # Try baseline first (most accurate)
    baseline_path = os.path.join(input_dir, 'baseline_90d_daily.json')
    if os.path.exists(baseline_path):
        with open(baseline_path, 'r', encoding='utf-8') as f:
            baseline_data = json.load(f)
            if 'metadata' in baseline_data:
                reference_date = baseline_data['metadata'].get('reference_date')
    
    # Fallback to hybrid files
    if not reference_date:
        for period_file in ['hybrid_data_90d.json', 'hybrid_data_30d.json', 'hybrid_data_7d.json']:
            period_path = os.path.join(input_dir, period_file)
            if os.path.exists(period_path):
                with open(period_path, 'r', encoding='utf-8') as f:
                    period_data = json.load(f)
                    if 'metadata' in period_data:
                        reference_date = period_data['metadata'].get('reference_date')
                        break
